...
def print_recourse_audit_report(X, y, df, group_by = ['y']):
    """
    :param X: features
    :param y: true labels
    :param df: DataFrame containing cost and feasibility of recourse, produced by RecourseAuditor
    :param group_by:
    :return:
    """
...
def subset_data_and_plot(y, data, group_by = ['y']):
    ...
    if 'y' in group_by:
        ...
        sns.violinplot(
                ...
                inner = 'quartile').set(
                title = "Histogram of Cost of Recourse by %s for Y=%s" % (group_by[0], y), xlabel = group_by[0])